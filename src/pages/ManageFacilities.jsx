...
function ManageFacilities() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5000/facilities/owner/${user.email}`, { withCredentials: true })
      .then(res => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/facilities/${id}`, { withCredentials: true })
  ...
  const handleUpdate = async (e) => {
    e.preventDefault()
    try {
      await axios.put(`http://localhost:5000/facilities/${editFacility._id}`, editFacility, { withCredentials: true })
